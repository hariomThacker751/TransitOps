...
import {
  VEHICLE_STATUS, VEHICLE_STATUS_OPTIONS, VEHICLE_TYPES, VEHICLE_REGIONS, resolveStatus,
                ...
                <div className="space-y-1.5">
                  {trips.slice(0, 5).map((t) => (
                    <div key={t.trip_id} className="flex items-center justify-between rounded-lg border border-ink-100 px-3 py-2 text-xs">
                      ...
                      <StatusBadge status={resolveStatus(VEHICLE_STATUS, t.status === 'Completed' ? { key: 'Completed', label: 'Completed', palette: 'emerald', dot: 'emerald' } : null)} />
                    </div>
                  ))}
                </div>
