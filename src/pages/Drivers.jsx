...
import {
  DRIVER_STATUS, DRIVER_STATUS_OPTIONS, LICENSE_CATEGORIES, resolveStatus,
                ...
                <div className="space-y-1.5">
                  {trips.slice(0, 5).map((t) => (
                    <div key={t.trip_id} className="flex items-center justify-between rounded-lg border border-ink-100 px-3 py-2 text-xs">
                      ...
                      <StatusBadge status={resolveStatus(DRIVER_STATUS, t.status === 'Dispatched' ? { key: 'On Trip', label: 'On Trip', palette: 'blue', dot: 'blue' } : null)} />
                    </div>
                  ))}
                </div>
